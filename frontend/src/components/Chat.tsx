...
export const ChatBoxFloat = ({ gameId }: ChatBoxProps) => {
  const { iterateGameStream, saveGame, isFetching, gameCode } =
    useGameBuilder();
  ...
  return (
    <div className="flex w-full h-[102px] gap-5 justify-center items-center rounded-[16px] border border-[#F094FA]/20 bg-gradient-to-b from-white/10 to-black/10 px-[24.6px]! py-[10.6px]!">
      <input
        className={`w-full h-[50px] outline-none font-mono! rounded-lg px-3! py-3! text-sm! leading-6 text-zinc-300! placeholder:text-zinc-500! overflow-auto!`}
        placeholder="Add to the game..."
        value={message}
        onChange={(e) => setMessage(e.target.value)}
      />

      <div className="flex flex-col gap-2 mt-2! w-25 h-full mr-1! text-sm rounded-lg">
        <ThemeButton
          bg="#0081e2"
          hover={`hover:cursor-pointer! hover:bg-[#0081e9] `}
          radius="rounded-lg!"
          padding="py-1! px-4!"
          onClick={handleSend}
        >
