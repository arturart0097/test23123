import type { GameModel } from "../contexts/Types";
...
export async function installSDK(gameCode: string): Promise<{ code: string }> {
  const res = await fetchWithAuth(`${API_ENDPOINT}/api/install-sdk`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  if (!res.ok) {
    throw new Error(`SDK installation failed: ${res.statusText}`);
  }
...
export async function suggestQuests(
  ...
  const res = await fetchWithAuth(`${API_ENDPOINT}/api/suggest-quests`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ game_code: gameCode }),
  });
  ...
  if (!res.ok) {
    throw new Error(`Quest suggestion failed: ${res.statusText}`);
  }

  const response = await res.json();

  return JSON.parse(response.quests);
}

export async function implementQuests(
  gameCode: string,
  questDescriptions: string[]
): Promise<{code: string}> {
  const res = await fetchWithAuth(`${API_ENDPOINT}/api/implement-quests`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ game_code: gameCode, quests: questDescriptions }),
  });

  if (!res.ok) {
    throw new Error(`Quest implementation failed: ${res.statusText}`);
  }
...
export async function generateCodePublish(
  ...
  const res = await fetchWithAuth(
    `${API_ENDPOINT}/api/generate-code-publish?model=${model}`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ messages, game_id: game_id, LLM: LLM }),
    },
  );
...
export async function listProjectGuest(gameID: number): Promise<Project[]> {
  const res = await fetch(`${API_ENDPOINT}/api/projects?id=${gameID}`, {
    headers: { Authorization: "Bearer GUEST" },
  });
  if (!res.ok) {
    throw new Error(`Failed to fetch project (guest): ${res.statusText}`);
  }
  return (await res.json()) as Project[];
}
...
export async function saveGameGenres(
  gameID: number,
  genres: string[],
): Promise<boolean> {
  const res = await fetchWithAuth(`${API_ENDPOINT}/api/projects/genres`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      game_id: gameID,
  ...
  if (!res.ok) {
    throw new Error(`Failed to save game genres: ${res.statusText}`);
  }

  return res.ok;
}

export async function saveGameOrientation(
  gameID: number,
  orientation: string,
): Promise<boolean> {
  const res = await fetchWithAuth(`${API_ENDPOINT}/api/projects/orientation`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      game_id: gameID,
  ...
  if (!res.ok) {
    throw new Error(`Failed to save game orientation: ${res.statusText}`);
  }

  return res.ok;
}

export async function updateSDKVersion(
  gameID: number,
  sdkVersion: number,
): Promise<boolean> {
  const res = await fetchWithAuth(`${API_ENDPOINT}/api/projects/sdkversion`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      game_id: gameID,
  ...
  if (!res.ok) {
    throw new Error(`Failed to update SDK version: ${res.statusText}`);
  }
...
export async function updateQuests(
  ...
  const res = await fetchWithAuth(`${API_ENDPOINT}/api/projects/quests`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      game_id: gameID,
  ...
  if (!res.ok) {
    throw new Error(`Failed to update quests: ${res.statusText}`);
  }

  return res.ok;
}

export async function submitGameForApproval(
  gameID: number
): Promise<boolean> {
  const res = await fetchWithAuth(`${API_ENDPOINT}/api/projects/submit`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      id: gameID,
  ...
  if (!res.ok) {
    throw new Error(`Failed to submit game for approval: ${res.statusText}`);
  }
...
export async function IterateGame(
  ...
  const newCodeRes = await fetchWithAuth(
    `${API_ENDPOINT}/api/projects/iterate`,
    {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ convo_id: convo_id, model: model, LLM: LLM }),
    },
  );
...
export async function ChatFlow(
  ...
  return PostMessage(convo_id, message).then(async (res) => {
    ...
    const data = await IterateGame(convo_id, model, LLM);
    return data;
  });
}
