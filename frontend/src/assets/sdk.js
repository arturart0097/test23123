(function (global) {
  ...
  function _getPrismAccessToken() {
    try {
      if (
        global.prism &&
        global.prism.accessToken &&
        typeof global.prism.accessToken === "string"
      ) {
  ...
  GameGPTSDK.prototype.initiateWager = async function (amount) {
    if (!this._ready) {
      const err = new Error(
        "SDK not ready. Call init() with backendEndpoint first."
      );
      this.events.emit("error", err);
      throw err;
    }
    amount =
      typeof amount === "number" ? amount : this._config.defaultWagerAmount;
    const userId = this.user && this.user.id;
    if (!userId) {
      const err = new Error(
        "No user id (sub) available on accessToken. Call setAccessToken(jwt) or ensure window.prism.accessToken is available."
      );
      this.events.emit("error", err);
      throw err;
    }
  ...
  GameGPTSDK.prototype._postScoreInternal = async function ({
    score,
    gameName,
  }) {
  ...
  // convenience aliases
  GameGPTSDK.prototype.on = function (evt, fn) {
    return this.events.on(evt, fn);
  };
  GameGPTSDK.prototype.off = function (evt, fn) {
    return this.events.off(evt, fn);
  };
  GameGPTSDK.prototype.once = function (evt, fn) {
    return this.events.once(evt, fn);
  };
  GameGPTSDK.prototype.emit = function (evt, ...args) {
    return this.events.emit(evt, ...args);
  };
  const instance = new GameGPTSDK();

  GameGPTSDK.ready = function (callback) {
    if (typeof callback === "function") {
      if (instance._ready) {
        callback(instance);
      } else {
        instance.on("ready", () => callback(instance));
      }
    }
  };

  GameGPTSDK.gameStart = function (callback) {
    if (typeof callback === "function") {
      instance.on("game_start", () => callback(instance));
    }
  };

  GameGPTSDK.gameOver = function (callback) {
    if (typeof callback === "function") {
      instance.on("game_over", () => callback(instance));
    }
  };

  GameGPTSDK.playAgain = function (callback) {
    if (typeof callback === "function") {
      instance.on("play_again", () => callback(instance));
    }
  };

  if (!global.GameGPTSDK) {
    global.GameGPTSDK = GameGPTSDK;
    global.gameGPT = instance;
  }
